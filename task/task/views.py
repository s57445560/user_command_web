...
class Index(View):
    ...
    def get(self, request):
        ...
        if request.GET.get("status"):
            ...
            return HttpResponse(json.dumps({"data": for_list}))
...
class User_edit(View):
    ...
    def post(self, request):
        ...
        print(id, username, passwd, status)
        user_obj = models.User.objects.filter(id=id).first()
        # 如果都相等证明没有变化 直接返回。
        if user_obj.passwd == passwd and dic[user_obj.status] == status:
            return HttpResponse(json.dumps({"message": "", "status": user_obj.status, "passwd": passwd}))
...
class Host_info(View):
    ...
    def post(self, request):
        ...
        if currentPage and handlesize:
            if pt:
                host_objs = models.Hosts.objects.get_queryset().order_by('id').filter(group_id_id=pt).values('ip', 'memory', 'group_id__name',
...
class Task_info(View):
    ...
    def post(self, request):
        ...
        if request.POST.get("status") == "GET_ERROR":
            group_objs = models.Task_result.objects.filter(task_id=task_id, task_status=False).values("group_name").annotate(
                c=Count('group_name'))
            for group in group_objs:
                result_idc = {}
                result_idc["name"] = group["group_name"]
                result_idc["list"] = []
                task_objs = models.Task_result.objects.filter(group_name=group["group_name"], task_id_id=task_id,task_status=False).all()
                for task in task_objs:
                    task_dic = {}
                    task_dic[task.ip] = task.message
                    result_idc["list"].append(task_dic)
